...
public:
	...
	void set_abs_angles(c_qangle angle)
	{
		static auto set_abs_angles_fn = reinterpret_cast<void(__thiscall*)(c_base_entity*, const c_qangle&)>(
			sig("client.dll", "55 8B EC 83 E4 F8 83 EC 64 53 56 57 8B F1")); // old 55 8B EC 83 E4 F8 83 EC 64 53 56 57 8B F1 E8
	...
	{
		writable_bones() = 0;
		static auto invalidate_bone_cache = sig("client.dll", "80 3D ? ? ? ? ? 74 16 A1 ? ? ? ? 48 C7 81");
	...
	bool is_breakable()
	{
		...
		*/
		...
		const int take_damage = get_take_damage();
		client_class* cclass = get_client_class(); // ok
		...
		static IsBreakableEntity_t  IsBreakableEntityEx = (IsBreakableEntity_t)sig("client.dll", "55 8B EC 51 56 8B F1 85 F6 74 68 83 BE");

		result = IsBreakableEntityEx(this);
...
/*	bool is_dormant()
	...
	*/
	...
	c_vector3d get_bone_position(int i)
	{
		...
		if (this->setup_bones(matrix, 128, 0x00000100, global_vars_base->curtime))
		{
	...
	{
		static auto invalidate_bone_bache = sig("client.dll", "80 3D ? ? ? ? ? 74 16 A1 ? ? ? ? 48 C7 81") + 10;
	...
	offset(get_take_damage(), int32_t, 0x280)
