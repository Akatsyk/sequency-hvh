...
{
	static const auto ik_init_address = sig("client.dll", "55 8B EC 83 EC 08 8B 45 08 56 57 8B F9 8D 8F");
...
{
	static const auto update_targets_address = sig("client.dll", "55 8B EC 83 E4 F0 81 EC ? ? ? ? 33 D2");
...
{
	static const auto solve_dependencies_address = sig("client.dll", "55 8B EC 83 E4 F0 81 EC ? ? ? ? 8B 81");
...
{
	static const auto xref_address = sig("client.dll", 
...
{
	static auto fn = reinterpret_cast<bool(__thiscall*)(void*, int)>(sig("client.dll", "55 8B EC 83 EC 10 53 56 57 8B F9 8B 87"));
...
{
	static auto fn = reinterpret_cast<void(__thiscall*)(int)>(sig("client.dll", "55 8B EC 56 57 8B F9 8B B7 ? ? ? ? 8B C6"));
...
std::optional<c_vector3d> c_cs_player::get_hitbox_position(hitbox id, matrix3x4* bones) const
{
	...
	if (studio_model) {
		...
		if (hitbox)
		{
			...
			c_vector3d max, min;
			math::vector_transform(hitbox->bbmax + mod, bones[hitbox->bone], max);
			math::vector_transform(hitbox->bbmin - mod, bones[hitbox->bone], min);
...
/*
c_vector3d c_cs_player::get_eye_pos() {
...
bool c_cs_player::is_shooting(c_user_cmd* cmd, const float time)
{
	...
	if (weapon->is_grenade())
		...
		return (cmd->buttons & c_user_cmd::attack || cmd->buttons & c_user_cmd::attack2) && can_shoot(cmd, time, weapon);
	else
		return cmd->buttons & c_user_cmd::attack && can_shoot(cmd, time, weapon);
}

bool c_cs_player::can_shoot(c_user_cmd* cmd, const float time, c_base_combat_weapon* weapon, const bool manual_shot)
{
...
bool c_cs_player::is_enemy()
{
	...
	if (!local /*|| this == nullptr*/)
		return false;

	return local->get_team() != get_team();
}

bool c_cs_player::is_enemy2()
{
	const auto local = get_local_player();
