...
{
	const auto local = c_cs_player::get_local_player();
	if (!local) return _m_flCustomBloomScale(data, entity, output);

	static float iBloomValue = 1.f;
...
{
	const auto local = c_cs_player::get_local_player();
	if (!local) return _m_bUseCustomAutoExposureMin(data, entity, output);
...
{
	const auto local = c_cs_player::get_local_player();
	if (!local) return _m_bUseCustomAutoExposureMax(data, entity, output);
...
{
	const auto local = c_cs_player::get_local_player();
	if (!local) return _m_flCustomAutoExposureMin(data, entity, output);

	static float iBloomValue = 1.f;
	if (iBloomValue != config->Visuals.bloom_cvar_scale) 
	{
		data->value.m_Float = config->Visuals.bloom_expose  *0.001f;
...
{
	const auto local = c_cs_player::get_local_player();
	if (!local) return _m_flCustomAutoExposureMax(data, entity, output);
