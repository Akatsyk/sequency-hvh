...
public:
	...
	static c_vector3d cross_products(const c_vector3d& a, const c_vector3d& b)
	{
		return c_vector3d(a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x);
	}

	__forceinline static void vector_angles(const c_vector3d& forward, const c_vector3d& pseudoup, c_vector3d& angles) {
		c_vector3d left = cross_products(pseudoup, forward);
		...
		{
			angles.x = atan2f(-forward.z, forward_dist) * 180 / 3.14159265358979323846f;
	...
	__forceinline static void angle_vector(const c_vector3d& angles, c_vector3d* forward) {
		float sp, sy, cp, cy;
	...
	__forceinline static void sincos(float radians, float* sine, float* cosine) {
		__asm {
	...
	__forceinline static void angle_vector___(c_vector3d q, c_vector3d& vec) {
		float sp, sy, cp, cy;
	...
	*/
	...
	{
		static DWORD dwViewMatrix = (DWORD)sig(("client.dll"), "0F 10 05 ? ? ? ? 8D 85 ? ? ? ? B9");
	...
	inline static void fast_rsqrt(float a, float* out)
	{
	...
	__forceinline static float fast_vec_normalize(c_vector3d& vec)
	{
