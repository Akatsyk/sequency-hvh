...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags) {
	...
	if (first_begin_of_the_frame) {
		...
		if (window->Collapsed) {
			...
			window->BorderSize = (flags & ImGuiWindowFlags_ShowBorders) ? 0.15f : 0.0f;
...
bool ImGui::Tab(const char* label, bool toggled, const ImVec2& size_arg)
{
	...
	const ImU32 framecol = ImColor(0, 128, 128, 180);
	const ImU32 framecol2 = ImColor(0, 128, 128, color_alpha);

	if (toggled)
		RenderFrame(bb.Min, bb.Max, framecol, toggled); //style.FrameRounding); // maybe crash

	if (hovered)
		RenderFrame(bb.Min, bb.Max, framecol2, toggled);
