...
void c_miscellaneous::disable_post_processing() {
	...
	static auto& disable_post_processing_override = **reinterpret_cast<bool**>(reinterpret_cast<uint32_t>(sig("client.dll", "83 EC 4C 80 3D") + 5));
...
void c_miscellaneous::unlock_client_command_buffer() {
	...
	static const auto new_commands = reinterpret_cast<uint8_t*>(reinterpret_cast<uint32_t>(sig("engine.dll", "55 8B EC A1 ? ? ? ? 81 EC ? ? ? ? B9 ? ? ? ? 53 8B 98")) + 0xbd);
	...
	NtProtectVirtualMemory ntVMemoryFunc = (NtProtectVirtualMemory)GetProcAddress(GetModuleHandleA("ntdll.dll"), "NtProtectVirtualMemory");
	ntVMemoryFunc(current_process, &address, &size, PAGE_READWRITE, &old);
	address = new_commands;
	size = sizeof(uintptr_t);
	*new_commands = config->Ragebot.enable ? 24 : 15;
	ntVMemoryFunc(current_process, &address, &size, old, &old);
...
void c_miscellaneous::remove_visual_recoil(c_view_setup& view) {
	if (!engine_client()->is_ingame() || !engine_client()->is_connected() || !c_cs_player::get_local_player() || !c_cs_player::get_local_player()->is_alive())return;
	const auto local = c_cs_player::get_local_player();
	if (!local || !local->is_alive() || input->camera_in_third_person)return;
...
void c_miscellaneous::set_viewmodel_parameters()
{
	...
	{
		if (weapon_debug_spread_show && local->is_scoped())
			weapon_debug_spread_show->set_value(0);
		else if (weapon_debug_spread_show)
		{
...
void c_miscellaneous::remove_smoke() {
	...
	*smoke_count = 0;
	...
	const auto Smokefire = material_system()->find_material("particle/vistasmokev1/vistasmokev1_fire");
	...
	if (!Smokegrenade || Smokegrenade->is_error_material() ||
		...
		!Smokefire || Smokefire->is_error_material())
	{
		return;
	}
	...
	Smokefire->set_material_var_flag(material_var_no_draw, config->Visuals.nosmoke);
}
...
void c_miscellaneous::misc_cm(c_user_cmd* cmd) {
	...
	if (c_cs_player::get_local_player()->is_alive()) {
		...
		static auto current_hand_pos = cvar()->find_var("cl_righthand")->value;
...
//			prediction_system->repredict(local, cmd);
		...
		const auto value = cvar()->find_var("sv_showimpacts");
		...
		const auto  cl_grenadepreview = cvar()->find_var("cl_grenadepreview");
	...
	if (iCurrentTime != iLastTime) {
		...
		if (config->Misc.clantag) {
			...
			ChangeClantag(tag, "paralys.is");
		}
		else
			ChangeClantag("", "paralys.is");
	}
...
void c_miscellaneous::modify_player_model() {
	...
	if (local) {
		...
		if (player_model.length() > 0 && model_info_client()) {
			...
			if (model_index)
				local->set_model_index(model_index);
		}
	}
}
...
void c_miscellaneous::preserve_killfeed() {
	...
	if (c_cs_player::get_local_player())
	{
		...
		const DWORD* deathNotice = FindHudElement<DWORD>("CCSGO_HudDeathNotice"); //Check client.dll & reallocate after game change xd
...
void c_miscellaneous::shift_ticks(c_user_cmd* cmd, bool send_packet) {
	...
	if (cfg.exploits == 0  
		|| weapon->get_item_definition() == weapon_revolver 
		|| info->WeaponType == weapontype_knife 
		|| info->WeaponType == weapontype_grenade 
		|| info->WeaponType == weapontype_c4 
		&& weapon->get_item_definition() == weapon_hegrenade 
		&& weapon->get_item_definition() == weapon_molotov 
		&& weapon->get_item_definition() == weapon_incgrenade 
	...
	static auto max_shift_ticks = 14;
	...
	if (cfg.exploits > 0 && !(c_cs_player::get_local_player()->get_flags() & c_cs_player::flags::frozen)) {
		auto recharge = []() {
			static auto needs_recharge = false;

			if (client_state->choked_commands > 4 || net_channel->choked_packets > 4) {
				needs_recharge = true;
			}
			...
			if (needs_recharge) {
				if (!GetAsyncKeyState(config->Ragebot.fakeduck)) {
					...
					_events.push_back(_event("exploits recharged!"));
				}
			}
		};
		recharge();
...
//	}
	...
	if (config->Misc.buy_bot_flashbang) {
		engine_client()->clientcmd_unrestricted(
			_("buy flashbang; "), 0);
	}
