...
#define global_color  c_color(18, 18, 18, 255)
...
void c_esp::indicator_ini() {
	...
	Drawing::DrawRect(position.x + 5, position.y + 5, 200 - 10, dheight - 10 - 5, c_color(18, 18, 18, 255));
	...
	Drawing::DrawRectGradientVertical(position.x + 8, position.y + 23, 200 - 16, 3, c_color(139, 0, 255, 255), c_color(0,255,0, 255));
	...
	if (config->Ragebot.fakelag_enable && config->Ragebot.enable) { // packet
		...
		float newvalue = antiaim->get_visual_choke();
		value = lerp(ind_speed, value, newvalue);
		draw_progressbar(position, "Fakelag", c, value);
	...
	{ // local velocity
		...
		if (maxvel) {
			...
			value = lerp(ind_speed, value, newvalue);
			draw_progressbar(position, "Speed", c, value);
		}
	}
	...
	/*{ //ping
		...
		value = lerp(ind_speed, value, rtt);
		draw_progressbar(position, "ping", c, value / 1.f);
...
void c_esp::adddmg(c_game_event* event) {
	...
	const auto attacker = client_entity_list()->get_client_entity(engine_client()->get_player_for_user_id(event->GetInt(_("attacker"))));
	const auto target = reinterpret_cast<c_cs_player*>(client_entity_list()->get_client_entity(engine_client()->get_player_for_user_id(event->GetInt(_("userid")))));
	...
	if (attacker && target && attacker == local && target->is_enemy())
	{
		...
		toadd.damage = event->GetInt(_("dmg_health"));
...
void c_esp::esp_keybinds() {
	...
	if (GetKeyState(config->Misc.damage_override))
		amount++;
	...
	if (config->Visuals.indicators_styles == 1)
		Drawing::DrawRectGradientVertical(pos.x + 8, pos.y + 23, 200 - 16, 3, c_color(139, 0, 255, 255), c_color(0, 255, 0, 255));
	...
	auto keybind__ = [&](const char* text, int key) -> void {
		auto size = Drawing::GetTextSize(F::ESP, text);
		int temp_width = size.right, temp_height = size.top;
		...
		if (GetKeyState(key)) {
			auto size2 = Drawing::GetTextSize(F::ESP, text);
			int width = size.right, height = size.top;
			if (config->Visuals.indicators_styles == 1) { //rectangles behind text, looks ugly af cyka
				Drawing::DrawRect(pos.x + 9, pos.y, 1, 20, c_color(config->Misc.menucolor[0] * 255, config->Misc.menucolor[1] * 255, config->Misc.menucolor[2] * 255, 255));
			}
			Drawing::DrawString(F::ESP, pos.x + 14, pos.y + 2, c_color(255, 255, 255, 155), false, text);
	...
	keybind__("Doubletap", config->Ragebot._auto.dt_key);
	keybind__("DamageOverride", config->Misc.damage_override);

	keybind_("Left", antiaim->_manual_yaw == -90 ? true : false);
    keybind_("Back", antiaim->_manual_yaw == 0.1 ? true : false);
    keybind_("Right", antiaim->_manual_yaw == 90 ? true : false);
...
void c_esp::render_watermark() {
	...
	sprintf(buffer, _("desync.vip | rtt: %.0fms | rate: %.0f | %s"), rtt, 1.f / global_vars_base->interval_per_tick, timebuff);
	...
	Drawing::DrawString(F::Menu, w - watermark_size.right - 15.f, 11.f, c_color(255, 255, 255, 170), false, buffer);
}
...
void c_esp::draw_enemy_impact(c_game_event* event)
{
	...
	const auto attacker = reinterpret_cast<c_cs_player*>(client_entity_list()->get_client_entity(engine_client()->get_player_for_user_id(event->GetInt(_("userid")))));
	if (!attacker)
		return;
	...
	c_vector3d end = c_vector3d(event->GetFloat("x"), event->GetFloat("y"), event->GetFloat("z"));
...
void draw_beam(c_vector3d Start, c_vector3d End, c_color color, float Width) {
	...
	info.type = 0;
	info.model_name = "sprites/white.vmt";
	info.model_index = -1;
	info.halo_scale = 0.f;
	info.life = 2.f;
	info.width = Width;
	...
	auto beam = view_render_beams->create_beam_points(info);
	if (beam) view_render_beams->draw_beam(beam);
}
...
void c_esp::draw_trails(c_cs_player* local)
{
	...
	if (old_origin.length2d() != origin.length2d() && local->is_alive())
...
{
	auto local = c_cs_player::get_local_player();
	if (!local || !config->Visuals.noscope || config->Visuals.enchance_scope)
		return;
...
void c_esp::fsn_nightmode() {
	...
	auto local = c_cs_player::get_local_player();
	if (!engine_client()->is_ingame() || !engine_client()->is_connected() || !local)
	{
		old_Skyname = "";
		OldSky = 0;
...
void c_esp::draw_spread_crosshair() {
	...
	if (config->Visuals.spread_crosshair == 1) {
		...
		for (int seed{ }; seed < 1024; ++seed) {
			math::random_seed(math::random_float(0, 1024));
...
{
	c_vector3d min, max, pos, pos3D, top, top3D; entity->get_render_bounds(min, max);
	pos3D = entity->get_abs_origin() - c_vector3d(0, 0, 10); top3D = pos3D + c_vector3d(0, 0, max.z + 10);
...
void c_esp::bomb_timer(c_cs_player* entity) {
	...
	const float real_time = c4_time - (global_vars_base->interval_per_tick * local->get_tick_base());
	char time_char[5];
...
bool night_mode()
{
	...
	if (!local_player || !engine_client()->is_connected() || !engine_client()->is_ingame())
	{
		old_Skyname = "";
		OldNightmode = false;
		OldSky = 0;
	...
	if (OldNightmode != config->Visuals.nightmode)
	{
		...
		for (MaterialHandle_t i = material_system()->FirstMaterial(); i != material_system()->InvalidMaterial(); i = material_system()->NextMaterial(i))
		{
			...
			{
				if (config->Visuals.nightmode == 1)
				{
					if (strstr(pMaterial->GetTextureGroupName(), "StaticProp"))
						pMaterial->ColorModulate(0.2f, 0.1f, 0.1f);
					else
						pMaterial->ColorModulate(0.1f, 0.2f, 0.2f);
...
void c_esp::render_player()
{
	...
	if (engine_client()->is_ingame() && local) {
		...
		client_entity_list()->for_each_player([&](c_cs_player* m_player) -> void
			{

				if (!m_player->is_enemy() || !m_player->is_alive())
					return;

				auto cfg = config->Visuals.enemy;

				if (m_player->get_team() == local->get_team() && m_player != local)
					cfg = config->Visuals.team;
				else if (m_player == local && input->camera_in_third_person)
				...
				if (cfg.visuals_enabled) {
					...
					//	c_vector3d min, max, pos, pos3D, top, top3D; m_player->get_render_bounds(min, max);
					//	pos3D = m_player->get_abs_origin_2() - c_vector3d(0, 0, 10); top3D = pos3D + c_vector3d(0, 0, max.z + 10);

					//	if (world_to_screen(pos3D, pos) && world_to_screen(top3D, top))
					//	{

					const float dt = ticks_to_time(local->get_tick_base()) - m_player->get_simtime();
...
void c_esp::render_panel()
{
	...
	if (engine_client()->is_ingame() && local) {
		for (auto i = 1; i < engine_client()->get_max_clients(); i++)
		{
			...
			if (m_player == nullptr ||
				...
				m_player->is_dormant() ||
				m_player == local ||
				m_player->get_team() == local->get_team()
				)
				continue;
