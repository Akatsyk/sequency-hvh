...
void c_damageesp::on_bullet_impact(c_game_event* event)
{
	...
	float x = event->GetFloat(_("x"));
	float y = event->GetFloat(_("y"));
	float z = event->GetFloat(_("z"));
...
void c_damageesp::on_player_hurt(c_game_event* event)
{
	...
	int dmg_health = event->GetInt(_("dmg_health"));
	int hitgroup = event->GetInt(_("hitgroup"));

	const auto entity = reinterpret_cast<c_cs_player*>(client_entity_list()->get_client_entity(engine_client()->get_player_for_user_id(event->GetInt(_("userid")))));
	const auto attacker = reinterpret_cast<c_cs_player*>(client_entity_list()->get_client_entity(engine_client()->get_player_for_user_id(event->GetInt(_("attacker")))));
