...
{
	if (!config->Visuals.hitmarker2d)return;
	linear_fade(hit_alpha, 0, 235, 255 / 1.7f, false);
	if (hit_alpha <= 0)return;

	int w, h;
	engine_client()->get_screen_size(w, h);

	Drawing::DrawLine((w / 2) - 10, (h / 2) - 8, (w / 2) - 4, (h / 2) - 3,c_color(255, 255, 255, hit_alpha));
	Drawing::DrawLine((w / 2) + 10, (h / 2) - 8, (w / 2) + 4, (h / 2) - 3,c_color(255, 255, 255, hit_alpha));
...
void c_hitmarker::on_player_hurt(c_game_event* event)
{
	...
	if (attacker && attacker == local) {
		if (target && target->is_enemy()) {
			...
			if (config->Visuals.hitsound == 2)
				PlaySoundA(hitsound_zabiv, NULL, SND_ASYNC | SND_MEMORY);
