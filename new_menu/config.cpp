...
void C_Config::Setup()
{
	...
	auto weapon_save = [&](Menu22::ragebot::weapon_config& _config) -> void {
		...
		SetupValue(_config.dt_key, 0, name + "dt_key");
	...
	SetupValue(config->Ragebot.variance, 0, "lag slowwalk");
	...
	save_color(config->Visuals.enemy.name_color, "enemy name color");
	...
	SetupValue(config->Visuals.view_setup_yoffset_enable, false, "view setup yoffset enable");
	...
	SetupValue(config->Misc.damage_override, 0, "damage_override");
	...
	SetupValue(config->Visuals.enemy.xqz_type, 0, "enemy xqz type");
	SetupValue(config->Visuals.team.xqz_type, 0, "desynclddt");
	...
	SetupValue(config->Visuals.desync2.xqz_type, 0, "desync xqz material type2");
	...
	SetupValue(config->Visuals.team.material_type, 0, "team material type");
	...
	SetupValue(config->Visuals.weapon2.preserve_model, false, "weapon preserve_model");
	SetupValue(config->Visuals.sleeve.preserve_model, false, "sleeve preserve_model");
	SetupValue(config->Visuals.sleeve2.preserve_model, false, "sleeve preserve_model");
	SetupValue(config->Visuals.arms.preserve_model, false,   "arms preserve_model");
	SetupValue(config->Visuals.arms2.preserve_model, false, "arms preserve_model");
	...
	SetupValue(config->Visuals.local2.preserve_model, false, "local preserve_model");
	SetupValue(config->Visuals.enemy.preserve_model, false, "enemy preserve_model");
	SetupValue(config->Visuals.enemy2.preserve_model, false, "enemy2 preserve_model");
	SetupValue(config->Visuals.team.preserve_model, false, "team preserve_model");
	SetupValue(config->Visuals.team.preserve_model, false, "team2 preserve_model");

	//enemy X, team X, local X, enemy2 X, team2 X, local2 X, weapon X, arms X, sleeve X, weapon2 X, arms2 X, sleeve2 X, backtrack X, desync X, desync2 X;

	SetupValue(config->Visuals.desync.pulse, false, "desync pulse");
	SetupValue(config->Visuals.desync2.pulse, false, "desync2 pulse");
	SetupValue(config->Visuals.weapon.pulse, false, "weapon pulse");
	SetupValue(config->Visuals.weapon2.pulse, false, "weapon2 pulse");
	SetupValue(config->Visuals.sleeve.pulse, false, "sleeve pulse");
	SetupValue(config->Visuals.sleeve2.pulse, false, "sleeve2 pulse");
	SetupValue(config->Visuals.arms.pulse, false, "arms pulse");
	SetupValue(config->Visuals.arms2.pulse, false, "arms2 pulse");
	SetupValue(config->Visuals.sleeve.pulse, false, "sleeve pulse");
	SetupValue(config->Visuals.sleeve2.pulse, false, "sleeve2 pulse");
	SetupValue(config->Visuals.backtrack.pulse, false, "backtrack pulse");
	SetupValue(config->Visuals.enemy.pulse, false, "enemy pulse");
	SetupValue(config->Visuals.enemy2.pulse, false, "enemy2 pulse");
	SetupValue(config->Visuals.team.pulse, false, "team pulse");
	SetupValue(config->Visuals.team2.pulse, false, "team2 pulse");
	SetupValue(config->Visuals.local.pulse, false, "local pulse");
	SetupValue(config->Visuals.local2.pulse, false, "local2 pulse");
...
void C_Config::Save(int idx)
{
	...
	{
		folder = std::string(path) + ("\\paralysis\\");

		switch (idx) {
		case 0:
			file = std::string(path) + ("\\paralysis\\D_1.ini");break;
		case 1:
			file = std::string(path) + ("\\paralysis\\D_2.ini");break;
		case 2:
			file = std::string(path) + ("\\paralysis\\D_3.ini");break;
		case 3:
			file = std::string(path) + ("\\paralysis\\D_4.ini");break;
		case 4:
			file = std::string(path) + ("\\paralysis\\D_5.ini");break;
		case 5:
			file = std::string(path) + ("\\paralysis\\D_6.ini");break;
		}
	}
